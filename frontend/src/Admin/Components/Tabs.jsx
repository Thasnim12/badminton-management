...
export default function ScrollableTabsButtonVisible() {
  ...
  const pendingBookings = data?.bookings?.filter(b => b.payment?.status === "Pending");
  const completedBookings = data?.bookings?.filter(b => b.payment?.status === "Completed");
  const failedBookings = data?.bookings?.filter(b => b.payment?.status === "Failed");
          ...
          {bookings.length > 0 ? (
            bookings.map((booking) => (
              <TableRow key={booking._id}>
                <TableCell align="center">{booking.payment?.razorpayOrderId}</TableCell>
                <TableCell align="center">{booking.user?.name || "N/A"}</TableCell>
                <TableCell align="center">{booking.court?.court_name || "N/A"}</TableCell>
                <TableCell align="center">
                  {booking.slot?.map(s => `${new Date(s.startTime).toLocaleTimeString()} - ${new Date(s.endTime).toLocaleTimeString()}`).join(", ") || "N/A"}
                </TableCell>
                <TableCell align="center">{new Date(booking.bookingDate).toLocaleDateString()}</TableCell>
  ...
  return (
    <Box sx={{ flexGrow: 1, bgcolor: "background.paper", margin: 5 }}>
      {/* Show loading state */}
      {isLoadingBookings ? (
        <Typography variant="h6" align="center">Loading bookings...</Typography>
      ) : (
        <>
          ...
          <Grid container spacing={3} sx={{ marginBottom: 3 }}>
            <Grid item xs={12} sm={3}>
              <Card>
                <CardContent>
                  <Typography variant="h6" align="center">Total Bookings</Typography>
                  <Typography variant="h4" align="center">{totalBookings || 0}</Typography>
                </CardContent>
              </Card>
            </Grid>
            <Grid item xs={12} sm={3}>
              <Card>
                <CardContent>
                  <Typography variant="h6" align="center">Pending Bookings</Typography>
                  <Typography variant="h4" align="center">{totalPending || 0}</Typography>
                </CardContent>
              </Card>
            </Grid>
            <Grid item xs={12} sm={3}>
              <Card>
                <CardContent>
                  <Typography variant="h6" align="center">Completed Bookings</Typography>
                  <Typography variant="h4" align="center">{totalCompleted || 0}</Typography>
                </CardContent>
              </Card>
            </Grid>
            <Grid item xs={12} sm={3}>
              <Card>
                <CardContent>
                  <Typography variant="h6" align="center">Failed Bookings</Typography>
                  <Typography variant="h4" align="center">{totalFailed || 0}</Typography>
                </CardContent>
              </Card>
            </Grid>
          </Grid>
